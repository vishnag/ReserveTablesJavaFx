...
public class OperationFrench {
		private Connection connection;
		public OperationFrench(Connection c) {
			this.connection = c;
		}
		...
		public ArrayList<Item> getMenu(String type) {
			String sql ="SELECT itemName, price, description FROM MenuFrench";
			if ( type != null) {
				sql += " WHERE type=?";
			}
			try {
		...
		public boolean addCustomer(String ln, String fn, String email) {
			String sql_addcustomer = "INSERT INTO customerFrench (email,lastName,firstName,updatedAt) VALUES (?,?,?,CURRENT_TIMESTAMP)";
		...
		public Customer getACustomer(int id) {
			String sql = "Select * From customerFrench where cid = ?";
		...
		public boolean reserveTable(int partySize,Date d,int tID , Customer c) {
			...
			String sql_reserve ="INSERT INTO ReservationFrench (reservationDate,partySize,cID,tID) values(?, ?, ?,?)";
		...
		public boolean cancelReservation(String email, Date d) {
			...
			if (cid != -1) {
				String queryCancelReservation ="DELETE FROM ReservationFrench WHERE cID=? AND reservationDate=?";
				try {
		...
		public boolean changeReservation(String email , Date date, int partySize) throws SQLException {
			...
			String query = "update reservationFrench set reservationDate = ?, partysize = ? where cid = ?";
		...
		public boolean rate(int stars, String feedback, Customer customer) throws SQLException {
			...
			String query = "INSERT INTO RatingFrench (cid, stars, feedback) VALUES (?,?,?) " +
		...
		public ArrayList<Rating> getRatingsAndFeedbacks() throws SQLException {
			String sql ="SELECT * FROM RatingFrench";
		...
		public ArrayList<ReservationInfo> getAllReservations() {
			String sql = "SELECT firstName, lastName, tID, partysize, seats, reservationDate\n" +
					"FROM ReservationFrench NATURAL JOIN Customer NATURAL JOIN aTableFrench";
		...
		public ArrayList<Person> getAllCustomersAndEmployees() {
			String sql = "SELECT firstname, lastname, email, updatedAt, discount, lastWorked\n" +
					"FROM customerFrench LEFT JOIN employeeFrench using(firstname, lastname, email)\n" +
					"UNION\n" +
					"SELECT firstname, lastname, email, updatedAt, discount, lastWorked\n" +
					"FROM customerFrench RIGHT JOIN employeeFrench using(firstname, lastname, email)";
			try {
				Statement statement = (Statement) connection.createStatement();
				ResultSet rs = statement.executeQuery(sql);
		...
		public ArrayList<Employee> getAllEmployees() {
			String sql = "SELECT * FROM EmployeeFrench";
...
//					em.setId(eid);
		...
		public ArrayList<Employee> getEmployeesWhoAreCustomers() {
			String sql = "select * from employeeFrench " +
					"where exists(" +
					"select * from customerFrench " +
					"where EmployeeFrench.firstName = customerFrench.firstname " +
					"AND employeeFrench.lastname = customerFrench.lastname)";
...
//					em.setId(id);
		...
		public ArrayList<Customer> getAllCustomers() {
			String sql = "SELECT * FROM CustomerFrench";
		...
		public ArrayList<Customer> getArchivedCustomers() {
			String sql = "SELECT * From arc_customerFrench";
			try {
				Statement statement = (Statement) connection.createStatement();
				ResultSet rs = statement.executeQuery(sql);
				ArrayList<Customer> list = new ArrayList<>();
...
//					customer.setId(id);
		...
		public ArrayList<Availability> getWeeklyAvailability() {
			String sql = "SELECT *\n" +
					"FROM (\n" +
					"SELECT reservationDate, (SELECT count(*) FROM aTableFrench) - count(*) as tablesAvailable\n" +
					"FROM ReservationFrench\n" +
					"WHERE reservationDate - CURDATE() < 7\n" +
					"GROUP BY reservationDate\n" +
					"UNION\n" +
					"SELECT adate, (SELECT count(*) FROM aTableFrench)\n" +
					"FROM dates\n" +
					"WHERE NOT EXISTS (\n" +
