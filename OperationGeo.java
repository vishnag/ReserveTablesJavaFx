...
public class OperationGeo {
		...
		public OperationGeo(Connection c) {
			this.connection = c;
		}
		...
		public ArrayList<Item> getMenu(String type) {
			String sql ="SELECT itemName, price, description FROM MenuGeo";
			if ( type != null) {
				sql += " WHERE type=?";
			}
			try {
		...
		public boolean addCustomer(String ln, String fn, String email) {
			String sql_addcustomer = "INSERT INTO customerGeo (email,lastName,firstName,updatedAt) VALUES (?,?,?,CURRENT_TIMESTAMP)";
		...
		public Customer getACustomer(int id) {
			String sql = "Select * From customerGeo where cid = ?";
		...
		public boolean reserveTable(int partySize,Date d,int tID , Customer c) {
			...
			String sql_reserve ="INSERT INTO ReservationGeo (reservationDate,partySize,cID,tID) values(?, ?, ?,?)";
		...
		public boolean cancelReservation(String email, Date d) {
			...
			if (cid != -1) {
				String queryCancelReservation ="DELETE FROM ReservationGeo WHERE cID=? AND reservationDate=?";
				try {
		...
		public boolean changeReservation(String email , Date date, int partySize) throws SQLException {
			...
			String query = "update reservationGeo set reservationDate = ?, partysize = ? where cid = ?";
		...
		public boolean rate(int stars, String feedback, Customer customer) throws SQLException {
			...
			String query = "INSERT INTO RatingGeo (cid, stars, feedback) VALUES (?,?,?) " +
		...
		public ArrayList<Rating> getRatingsAndFeedbacks() throws SQLException {
			String sql ="SELECT * FROM RatingGeo";
		...
		public ArrayList<ReservationInfo> getAllReservations() {
			String sql = "SELECT firstName, lastName, tID, partysize, seats, reservationDate\n" +
					"FROM ReservationGeo NATURAL JOIN Customer NATURAL JOIN aTableGeo";
		...
		public ArrayList<Person> getAllCustomersAndEmployees() {
			String sql = "SELECT firstname, lastname, email, updatedAt, discount, lastWorked\n" +
					"FROM customerGeo LEFT JOIN employeeGeo using(firstname, lastname, email)\n" +
					"UNION\n" +
					"SELECT firstname, lastname, email, updatedAt, discount, lastWorked\n" +
					"FROM customerGeo RIGHT JOIN employeeGeo using(firstname, lastname, email)";
			try {
				Statement statement = (Statement) connection.createStatement();
				ResultSet rs = statement.executeQuery(sql);
		...
		public ArrayList<Employee> getAllEmployees() {
			String sql = "SELECT * FROM EmployeeGeo";
...
//					em.setId(eid);
		...
		public ArrayList<Employee> getEmployeesWhoAreCustomers() {
			String sql = "select * from employeeGeo " +
					...
					"where EmployeeGeo.firstName = customerGeo.firstname " +
					"AND employeeGeo.lastname = customerGeo.lastname)";
...
//					em.setId(id);
		...
		public ArrayList<Customer> getAllCustomers() {
			String sql = "SELECT * FROM CustomerGeo";
		...
		public ArrayList<Customer> getArchivedCustomers() {
			String sql = "SELECT * From arc_customerGeo";
			try {
				Statement statement = (Statement) connection.createStatement();
				ResultSet rs = statement.executeQuery(sql);
				ArrayList<Customer> list = new ArrayList<>();
...
//					customer.setId(id);
		...
		public ArrayList<Availability> getWeeklyAvailability() {
			String sql = "SELECT *\n" +
					"FROM (\n" +
					"SELECT reservationDate, (SELECT count(*) FROM aTableGeo) - count(*) as tablesAvailable\n" +
					"FROM ReservationGeo\n" +
					"WHERE reservationDate - CURDATE() < 7\n" +
					"GROUP BY reservationDate\n" +
					"UNION\n" +
					"SELECT adate, (SELECT count(*) FROM aTableGeo)\n" +
					"FROM dates\n" +
					"WHERE NOT EXISTS (\n" +
