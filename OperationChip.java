...
public class OperationChip {
		private Connection connection;
		public OperationChip(Connection c) {
			this.connection = c;
		}
		...
		public ArrayList<Item> getMenu(String type) {
			String sql ="SELECT itemName, price, description FROM MenuChip";
			if ( type != null) {
				sql += " WHERE type=?";
			}
			try {
		...
		public boolean addCustomer(String ln, String fn, String email) {
			String sql_addcustomer = "INSERT INTO customerChip (email,lastName,firstName,updatedAt) VALUES (?,?,?,CURRENT_TIMESTAMP)";
		...
		public Customer getACustomer(int id) {
			String sql = "Select * From customerChip where cid = ?";
		...
		public boolean reserveTable(int partySize,Date d,int tID , Customer c) {
			...
			String sql_reserve ="INSERT INTO ReservationChip (reservationDate,partySize,cID,tID) values(?, ?, ?,?)";
		...
		public boolean cancelReservation(String email, Date d) {
			...
			if (cid != -1) {
				String queryCancelReservation ="DELETE FROM ReservationChip WHERE cID=? AND reservationDate=?";
				try {
		...
		public boolean changeReservation(String email , Date date, int partySize) throws SQLException {
			...
			String query = "update reservationChip set reservationDate = ?, partysize = ? where cid = ?";
		...
		public boolean rate(int stars, String feedback, Customer customer) throws SQLException {
			...
			String query = "INSERT INTO RatingChip (cid, stars, feedback) VALUES (?,?,?) " +
		...
		public ArrayList<Rating> getRatingsAndFeedbacks() throws SQLException {
			String sql ="SELECT * FROM RatingChip";
		...
		public ArrayList<ReservationInfo> getAllReservations() {
			String sql = "SELECT firstName, lastName, tID, partysize, seats, reservationDate\n" +
					"FROM ReservationChip NATURAL JOIN Customer NATURAL JOIN aTableChip";
		...
		public ArrayList<Person> getAllCustomersAndEmployees() {
			String sql = "SELECT firstname, lastname, email, updatedAt, discount, lastWorked\n" +
					"FROM customerChip LEFT JOIN employeeChip using(firstname, lastname, email)\n" +
					"UNION\n" +
					"SELECT firstname, lastname, email, updatedAt, discount, lastWorked\n" +
					"FROM customerChip RIGHT JOIN employeeChip using(firstname, lastname, email)";
			try {
				Statement statement = (Statement) connection.createStatement();
				ResultSet rs = statement.executeQuery(sql);
		...
		public ArrayList<Employee> getAllEmployees() {
			String sql = "SELECT * FROM EmployeeChip";
...
//					em.setId(eid);
		...
		public ArrayList<Employee> getEmployeesWhoAreCustomers() {
			String sql = "select * from employeeChip " +
					"where exists(" +
					"select * from customerChip " +
					"where EmployeeChip.firstName = customerChip.firstname " +
					"AND employeeChip.lastname = customerChip.lastname)";
...
//					em.setId(id);
		...
		public ArrayList<Customer> getAllCustomers() {
			String sql = "SELECT * FROM CustomerChip";
		...
		public ArrayList<Customer> getArchivedCustomers() {
			String sql = "SELECT * From arc_customerChip";
			try {
				Statement statement = (Statement) connection.createStatement();
				ResultSet rs = statement.executeQuery(sql);
				ArrayList<Customer> list = new ArrayList<>();
...
//					customer.setId(id);
		...
		public ArrayList<Availability> getWeeklyAvailability() {
			String sql = "SELECT *\n" +
					"FROM (\n" +
					"SELECT reservationDate, (SELECT count(*) FROM aTableChip) - count(*) as tablesAvailable\n" +
					"FROM ReservationChip\n" +
					"WHERE reservationDate - CURDATE() < 7\n" +
					"GROUP BY reservationDate\n" +
					"UNION\n" +
					"SELECT adate, (SELECT count(*) FROM aTableChip)\n" +
					"FROM dates\n" +
					"WHERE NOT EXISTS (\n" +
