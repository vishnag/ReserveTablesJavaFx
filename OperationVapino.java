...
public class OperationVapino {
		private Connection connection;
		public OperationVapino(Connection c) {
			this.connection = c;
		}
		...
		public ArrayList<Item> getMenu(String type) {
			String sql ="SELECT itemName, price, description FROM MenuVAPINO";
			if ( type != null) {
				sql += " WHERE type=?";
			}
			try {
		...
		public boolean addCustomer(String ln, String fn, String email) {
			String sql_addcustomer = "INSERT INTO customerVAPINO (email,lastName,firstName,updatedAt) VALUES (?,?,?,CURRENT_TIMESTAMP)";
		...
		public Customer getACustomer(int id) {
			String sql = "Select * From customerVAPINO where cid = ?";
		...
		public boolean reserveTable(int partySize,Date d,int tID , Customer c) {
			...
			String sql_reserve ="INSERT INTO ReservationVAPINO (reservationDate,partySize,cID,tID) values(?, ?, ?,?)";
		...
		public boolean cancelReservation(String email, Date d) {
			...
			if (cid != -1) {
				String queryCancelReservation ="DELETE FROM ReservationVAPINO WHERE cID=? AND reservationDate=?";
				try {
		...
		public boolean changeReservation(String email , Date date, int partySize) throws SQLException {
			...
			String query = "update reservationVAPINO set reservationDate = ?, partysize = ? where cid = ?";
		...
		public boolean rate(int stars, String feedback, Customer customer) throws SQLException {
			...
			String query = "INSERT INTO RatingVAPINO (cid, stars, feedback) VALUES (?,?,?) " +
		...
		public ArrayList<Rating> getRatingsAndFeedbacks() throws SQLException {
			String sql ="SELECT * FROM RatingVAPINO";
		...
		public ArrayList<ReservationInfo> getAllReservations() {
			String sql = "SELECT firstName, lastName, tID, partysize, seats, reservationDate\n" +
					"FROM ReservationVAPINO NATURAL JOIN Customer NATURAL JOIN aTableVAPINO";
		...
		public ArrayList<Person> getAllCustomersAndEmployees() {
			String sql = "SELECT firstname, lastname, email, updatedAt, discount, lastWorked\n" +
					"FROM customerVAPINO LEFT JOIN employeeVAPINO using(firstname, lastname, email)\n" +
					"UNION\n" +
					"SELECT firstname, lastname, email, updatedAt, discount, lastWorked\n" +
					"FROM customerVAPINO RIGHT JOIN employeeVAPINO using(firstname, lastname, email)";
			try {
				Statement statement = (Statement) connection.createStatement();
				ResultSet rs = statement.executeQuery(sql);
		...
		public ArrayList<Employee> getAllEmployees() {
			String sql = "SELECT * FROM EmployeeVAPINO";
...
//					em.setId(eid);
		...
		public ArrayList<Employee> getEmployeesWhoAreCustomers() {
			String sql = "select * from employeeVAPINO " +
					"where exists(" +
					"select * from customerVAPINO " +
					"where EmployeeVAPINO.firstName = customerVAPINO.firstname " +
					"AND employeeVAPINO.lastname = customerVAPINO.lastname)";
...
//					em.setId(id);
		...
		public ArrayList<Customer> getAllCustomers() {
			String sql = "SELECT * FROM CustomerVAPINO";
		...
		public ArrayList<Customer> getArchivedCustomers() {
			String sql = "SELECT * From arc_customerVAPINO";
			try {
				Statement statement = (Statement) connection.createStatement();
				ResultSet rs = statement.executeQuery(sql);
				ArrayList<Customer> list = new ArrayList<>();
...
//					customer.setId(id);
		...
		public ArrayList<Availability> getWeeklyAvailability() {
			String sql = "SELECT *\n" +
					"FROM (\n" +
					"SELECT reservationDate, (SELECT count(*) FROM aTableVAPINO) - count(*) as tablesAvailable\n" +
					"FROM ReservationVAPINO\n" +
					"WHERE reservationDate - CURDATE() < 7\n" +
					"GROUP BY reservationDate\n" +
					"UNION\n" +
					"SELECT adate, (SELECT count(*) FROM aTableVAPINO)\n" +
					"FROM dates\n" +
					"WHERE NOT EXISTS (\n" +
